param(
    ...
    [Parameter(Mandatory = $false)]
    [String]$WorkerName = "ID=NemosMinerClassic", 
    [Parameter(Mandatory = $false)]
    [Int]$API_ID = 0, 
    [Parameter(Mandatory = $false)]
    [String]$API_Key = "", 
...
https://github.com/nemosminer/NemosMinerClassic/blob/master/LICENSE
NemosMinerClassic
"@
...
try {
    $Version = Invoke-WebRequest "https://raw.githubusercontent.com/nemosminer/NemosMinerClassic/InfoLookup/master/version.json" -UseBasicParsing -Headers @{"Cache-Control" = "no-cache"} | ConvertFrom-Json 
}
catch {  }
...
while ($true) {
    ...
    if ((Get-Date).AddDays(-1).AddMinutes($Donate) -ge $LastDonated -and ($Wallet -eq $WalletBackup -or $UserName -eq $UserNameBackup)) {
        ...
        if ($Donation) {
            ...
            if ($WorkerName) {$WorkerName = "NemosMinerClassic"}
        }
    }
